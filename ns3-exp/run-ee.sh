...
for sr in 100 200 500 1000 2000 5000 10000
do
	for M in 1 2 3
	do
		...
		cp "./bag-ee/sr_ICIOT_${M}_${sr}_0.txt" ./sr_loc.txt
		cp "./bag-ee/gw_ICIOT_${M}_${sr}_0.txt" ./gw_loc.txt
		cp "./bag-ee/pl_ICIOT_${M}_${sr}_0.txt" ./pl_mat.txt
		destlog=log
		start=`date +%s`
		./waf --run "adr --MType=Confirmed --MatrixPLEnabled" > "$destlog" 2>&1
		end=`date +%s`
		...
		echo "$runtime" >> "$destlog"
		#mv ./nodeData.txt ./res/nodeDataPL-GP.txt
		mv ./nodeEE.txt "./res/nodeEE_ICIOT_${M}_${sr}.txt"
		#mv ./globalPerformance.txt ./res/globalPerformancePL-GP.txt
		#mv ./phyPerformance.txt ./res/phyPerformancePL-GP.txt
	done
done
